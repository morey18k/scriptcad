...
def make_ohmics_1(um, p):
    ...
    for k in range(len(p.ypos)):
        ...
        point4 = db.Point(-(0.75*p.dohmicsx-0.70*p.padsize+10)*um, yfs[k]*um)
...
def make_island(um, p):
    ...
    if p.hastrench:
        trench_box = db.Box(p.connector_widthX*um, p.connectorSize*um)
        island_box = db.Box(p.island_sizeX*um, p.island_sizeY*um)
    else:
        trench_box = db.Box(p.connector_widthX*um, p.island_sizeY*um)
        island_box = db.Box(p.connector_widthX*um, p.island_sizeY*um)
    ...
    for k in range(len(spacings)):
        ...
        island_region.insert(trench_box.moved(0, p.ypos[k]*um))
        island_region.insert(connector_box.moved(-0.5*p.connector_widthX*um-0.4999*connector_width*um, p.ypos[k]*um))
        island_region.insert(connector_box.moved(0.5*p.connector_widthX*um+0.4999*connector_width*um, p.ypos[k]*um))
