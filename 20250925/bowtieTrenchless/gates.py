...
def create_finegates(um, p):
    ...
    path1 = db.Path([db.Point((lgate+0.5*p.qpcwidth-p.finegatewidth)*um, (0.5*p.dot_length)*um), db.Point(0.5*p.gate_separation*um, 0.5*p.gate_separation*um)], width = p.finegatewidth*um)
...
def create_gatepads(um, p):
    ...
    cgate_rgn = db.Region()
    path1 = db.Path([db.Point((0.5*p.gate_separation-1)*um, (0.5*p.gate_separation-1)*um), db.Point((p.gate_separation)*um, (p.gate_separation)*um)], width = p.coursegatewidth*um)
    ...
    portsr1 = rtop.ports.filter(orientation=0) + rbottom.ports.filter(orientation=0)
    portsr2 = rpadtop.ports.filter(orientation=180) + rpadbottom.ports.filter(orientation=180)

    routes = gf.routing.route_bundle(
        c,
        portsr1,
        portsr2,
        separation = 2*p.gate_separation,
        allow_width_mismatch = True,
        route_width = 1.0,
        cross_section = "metal_routing",
        waypoints = [(p.gatepadx*0.5, p.gate_separation), (p.gatepadx*0.6, p.gate_separation)]
    )
    ...
    portsl1 = ltop.ports.filter(orientation=180)  + lbottom.ports.filter(orientation=180)
    portsl2 = lpadtop.ports.filter(orientation=0) + lpadbottom.ports.filter(orientation=0)
