...
import sys
from semiSens.mesa_dot import make_mesa_gatepads
...
def create_patches(um, p):
    ...
    if p.has_island and p.island_sizeX >=0.7:
...
def create_finegates(um, p):
    ...
    finegates_region += cs_qpc.transformed(db.ICplxTrans.new(1, 0, False, (p.dot_length+p.finegatewidth+p.qpcwidth)*um, -0.8*p.dot_length*um))
    finegates_region += cs_qpc.transformed(db.ICplxTrans.new(1, 0, False, (p.dot_length+p.finegatewidth+p.qpcwidth)*um, 0.8*p.dot_length*um))

    finegates_region += cs_qpc.transformed(db.ICplxTrans.new(1, 90, False, (p.dot_length+8*p.finegatewidth+1*p.qpcwidth)*um, -0.3*p.dot_length*um))
    finegates_region += cs_qpc.transformed(db.ICplxTrans.new(1, 90, False, (p.dot_length+8*p.finegatewidth+1*p.qpcwidth)*um, 0.3*p.dot_length*um))
    ...
    path4 = db.Path([db.Point((p.dot_length+2*l_cs+p.qpcwidth-0.75*p.finegatewidth)*um, (0.8*p.dot_length)*um), 
                     db.Point(3*p.dot_length*um, (1.75*p.dot_length)*um),
                     db.Point(0.5*p.gate_separation*um, 0.5*p.gate_separation*um), 
                     db.Point(1.0*p.gate_separation*um, 1.5*p.gate_separation*um),
                     ], width = p.finegatewidth*um)

    path5 = db.Path([db.Point((p.dot_length+8*p.finegatewidth+1*p.qpcwidth)*um, (0.3*p.dot_length+l_cs+p.finegatewidth)*um), 
                     db.Point(4*p.dot_length*um, (1.3*p.dot_length)*um),
                     db.Point(1*p.gate_separation*um, 0.5*p.gate_separation*um), 
                     db.Point(1.5*p.gate_separation*um, 1.*p.gate_separation*um),
                     ], width = p.finegatewidth*um)
    
    path6 = db.Path([db.Point((p.dot_length+7*p.finegatewidth+1*p.qpcwidth)*um, (0.3*p.dot_length+l_cs+3*p.finegatewidth)*um), 
                     db.Point(3.5*p.dot_length*um, (1.5*p.dot_length)*um),
                     db.Point(0.75*p.gate_separation*um, 0.5*p.gate_separation*um), 
                     db.Point(1.25*p.gate_separation*um, 1.25*p.gate_separation*um),
                     ], width = p.finegatewidth*um)
    
    finegates_region.insert(path4)
    finegates_region.insert(path4.transformed(db.ICplxTrans.new(1, 0, True, 0, 0)))

    finegates_region.insert(path5)
    finegates_region.insert(path6)


    finegates_region.insert(path5.transformed(db.ICplxTrans.new(1, 0, True, 0, 0)))
    finegates_region.insert(path6.transformed(db.ICplxTrans.new(1, 0, True, 0, 0)))
...
def create_finegates_med(um, p):
    ...
    alt_plunger_region = db.Region()
    alt_plunger = db.Box(3.2*p.dot_length*um, p.finegatewidth*um)
    alt_plunger_region.insert(alt_plunger)
    alt_plunger_region.round_corners(0.1*um, 0.1*um, 32)
    if p.has_island:
        finegates_region += alt_plunger_region.transformed(db.ICplxTrans.new(1, 0, False, -(p.dot_length+2.1*p.finegatewidth)*um, -(0.5*p.dot_length*um)))
        finegates_region += alt_plunger_region.transformed(db.ICplxTrans.new(1, 0, False, -(p.dot_length+2.1*p.finegatewidth)*um, 0.5*p.dot_length*um))
    ...
    l_cs = 3*p.finegatewidth
    cs_qpc = create_qpc(p.qpcwidth, height_gate = p.finegatewidth, length_gate= l_cs)
    finegates_region += cs_qpc.transformed(db.ICplxTrans.new(1, 0, False, (p.dot_length-5*p.finegatewidth+p.qpcwidth)*um, -0.8*p.dot_length*um))
    finegates_region += cs_qpc.transformed(db.ICplxTrans.new(1, 0, False, (p.dot_length-5*p.finegatewidth+p.qpcwidth)*um, 0.8*p.dot_length*um))

    finegates_region += cs_qpc.transformed(db.ICplxTrans.new(1, 90, False, (p.dot_length+1*p.finegatewidth+1*p.qpcwidth)*um, -0.3*p.dot_length*um))
    finegates_region += cs_qpc.transformed(db.ICplxTrans.new(1, 90, False, (p.dot_length+1*p.finegatewidth+1*p.qpcwidth)*um, 0.3*p.dot_length*um))
    ...
    path4 = db.Path([db.Point((p.dot_length+2*l_cs+p.qpcwidth-7.75*p.finegatewidth)*um, (0.8*p.dot_length)*um), 
                     db.Point(3*p.dot_length*um, (1.75*p.dot_length)*um),
                     db.Point(0.5*p.gate_separation*um, 0.5*p.gate_separation*um), 
                     db.Point(1.0*p.gate_separation*um, 1.5*p.gate_separation*um),
                     ], width = p.finegatewidth*um)

    path5 = db.Path([db.Point((p.dot_length+1*p.finegatewidth+1*p.qpcwidth)*um, (0.3*p.dot_length+l_cs+p.finegatewidth)*um), 
                     db.Point(4*p.dot_length*um, (1.3*p.dot_length)*um),
                     db.Point(1*p.gate_separation*um, 0.5*p.gate_separation*um), 
                     db.Point(1.5*p.gate_separation*um, 1.*p.gate_separation*um),
                     ], width = p.finegatewidth*um)
    
    path6 = db.Path([db.Point((p.dot_length+0*p.finegatewidth+1*p.qpcwidth)*um, (0.3*p.dot_length+l_cs+2.75*p.finegatewidth)*um), 
                     db.Point(3.5*p.dot_length*um, (1.5*p.dot_length)*um),
                     db.Point(0.75*p.gate_separation*um, 0.5*p.gate_separation*um), 
                     db.Point(1.25*p.gate_separation*um, 1.25*p.gate_separation*um),
                     ], width = p.finegatewidth*um)
...
def create_gatepads(um, p):
    ...
    gate_trace_1 = db.Path([((-0.5*p.wHallbar)*um, 0.1*p.lHallbar*um),
                ((-0.4*p.dohmicsx)*um, 0.1*p.lHallbar*um),
                ((-0.4*p.dohmicsx)*um, 0.3*p.lHallbar*um),
                ((-0.58*p.dohmicsx)*um, 0.3*p.lHallbar*um),
                ((-0.58*p.dohmicsx)*um, 0.47*p.lHallbar*um),
    ...
    gate_trace_2 = db.Path([((-0.5*p.wHallbar)*um, 0.05*p.lHallbar*um),
                ((-0.55*p.dohmicsx)*um, 0.05*p.lHallbar*um),
                ((-0.55*p.dohmicsx)*um, 0.25*p.lHallbar*um),
                ((-0.75*p.dohmicsx)*um, 0.25*p.lHallbar*um)
                ], width = p.coursegatewidth*um)
    
    gate_trace_3 = db.Path([((-0.5*p.wHallbar)*um, 0*p.lHallbar*um),
                ((-0.75*p.dohmicsx)*um, 0*p.lHallbar*um)
                ], width = p.coursegatewidth*um)
    ...
    gate_trace_6 = db.Path([((+0.5*p.wHallbar)*um, -0.32*p.lHallbar*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector)*um, -0.32*p.lHallbar*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector)*um, (-0.20*p.lHallbar-0.20*p.dohmicsy)*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector+p.dohmicsx)*um, (-0.20*p.lHallbar-0.20*p.dohmicsy)*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector+p.dohmicsx)*um, (-0.45*p.lHallbar-0.20*p.dohmicsy)*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector+2*p.dohmicsx)*um, (-0.45*p.lHallbar-0.20*p.dohmicsy)*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector+2*p.dohmicsx)*um, (-0.55*p.lHallbar-0.20*p.dohmicsy)*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector+2.5*p.dohmicsx)*um, (-0.55*p.lHallbar-0.20*p.dohmicsy)*um)
    ], width = p.coursegatewidth*um)

    gate_trace_7 = db.Path([((+0.5*p.wHallbar)*um, -0.16*p.lHallbar*um),
                ((+0.5*p.wHallbar+0.5*p.w_connector)*um, -0.22*p.lHallbar*um),
                ((+0.5*p.wHallbar+0.5*p.w_connector)*um, -0.22*p.lHallbar*um),
                ((+0.5*p.wHallbar+p.w_connector+p.dohmicsx)*um,  -0.22*p.lHallbar*um),
                ((+0.5*p.wHallbar+p.w_connector+p.dohmicsx)*um, -0.22*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+2*p.dohmicsx)*um, -0.22*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+2*p.dohmicsx)*um, -0.22*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+2.5*p.dohmicsx)*um, -0.22*p.lHallbar*um)
    ], width = p.coursegatewidth*um)

    gate_trace_8 = db.Path([((+0.5*p.wHallbar)*um, -0.14*p.lHallbar*um),
                ((+0.5*p.wHallbar+0.5*p.w_connector)*um, -0.185*p.lHallbar*um),
                ((+0.5*p.wHallbar+0.5*p.w_connector)*um, -0.185*p.lHallbar*um),
                ((+0.5*p.wHallbar+p.w_connector+p.dohmicsx)*um,  -0.185*p.lHallbar*um),
                ((+0.5*p.wHallbar+p.w_connector+p.dohmicsx)*um, -0.185*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+2*p.dohmicsx)*um, -0.185*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+2*p.dohmicsx)*um, -0.185*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+2.5*p.dohmicsx)*um, -0.185*p.lHallbar*um),
                ((3.5*p.dohmicsx)*um, -0.185*p.lHallbar*um),
                ((3.5*p.dohmicsx)*um,-1.4*p.dohmicsy*um),
                ((3.7*p.dohmicsx)*um,-1.4*p.dohmicsy*um),
                ((3.7*p.dohmicsx)*um,-1.7*p.dohmicsy*um)
    ], width = p.coursegatewidth*um)

    gate_trace_9 = db.Path([((+0.5*p.wHallbar)*um, -0.12*p.lHallbar*um),
                ((+0.5*p.wHallbar+0.5*p.w_connector)*um, -0.15*p.lHallbar*um),
                ((+0.5*p.wHallbar+0.5*p.w_connector)*um, -0.15*p.lHallbar*um),
                ((+0.5*p.wHallbar+p.w_connector+p.dohmicsx)*um,  -0.15*p.lHallbar*um),
                ((+0.5*p.wHallbar+p.w_connector+p.dohmicsx)*um, -0.15*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+2*p.dohmicsx)*um, -0.15*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+2*p.dohmicsx)*um, -0.15*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+2.5*p.dohmicsx)*um, -0.15*p.lHallbar*um),
                ((4.0*p.dohmicsx)*um, -0.15*p.lHallbar*um),
                ((4.0*p.dohmicsx)*um, -0.25*p.lHallbar*um),
    ], width = p.coursegatewidth*um)


    gate_trace_10 = db.Path([((+0.5*p.wHallbar)*um, 0*p.lHallbar*um),
                ((+0.5*p.wHallbar+1.375*p.w_connector)*um, 0*p.lHallbar*um),
                ((+0.5*p.wHallbar+1.375*p.w_connector)*um, 0.11*p.lHallbar*um),
                ((+0.5*p.wHallbar+1.375*p.w_connector+p.dohmicsx)*um,  0.11*p.lHallbar*um),
                ((+0.5*p.wHallbar+1.375*p.w_connector+p.dohmicsx)*um, -0.11*p.lHallbar*um),
                ((+0.5*p.wHallbar+1.375*p.w_connector+2.5*p.dohmicsx)*um, -0.11*p.lHallbar*um),
    ], width = p.coursegatewidth*um)


    gate_trace_11 = db.Path([((+0.5*p.wHallbar)*um, 0.12*p.lHallbar*um),
                ((+0.5*p.wHallbar+p.w_connector)*um, 0.15*p.lHallbar*um),
                ((+0.5*p.wHallbar+p.w_connector)*um, 0.15*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+p.dohmicsx)*um,  0.15*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+p.dohmicsx)*um, 0.15*p.lHallbar*um),
                ((2.5*p.dohmicsx)*um, 0.15*p.lHallbar*um),
                ((2.5*p.dohmicsx)*um, 0.14*p.lHallbar*um),
                ((3.5*p.dohmicsx)*um, 0.14*p.lHallbar*um),
                ((4.0*p.dohmicsx)*um, 0.14*p.lHallbar*um),
                ((4.0*p.dohmicsx)*um, 0.25*p.lHallbar*um),
    ], width = p.coursegatewidth*um)

    gate_trace_12 = db.Path([((+0.5*p.wHallbar)*um, 0.14*p.lHallbar*um),
                ((+0.5*p.wHallbar+p.w_connector)*um, 0.185*p.lHallbar*um),
                ((+0.5*p.wHallbar+p.w_connector)*um, 0.185*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+p.dohmicsx)*um,  0.185*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+p.dohmicsx)*um, 0.185*p.lHallbar*um),
                ((2.5*p.dohmicsx)*um, 0.185*p.lHallbar*um),
                ((2.5*p.dohmicsx)*um, 0.175*p.lHallbar*um),
                ((3.5*p.dohmicsx)*um, 0.175*p.lHallbar*um),
                ((3.5*p.dohmicsx)*um,1.4*p.dohmicsy*um),
                ((3.7*p.dohmicsx)*um,1.4*p.dohmicsy*um),
                ((3.7*p.dohmicsx)*um,1.7*p.dohmicsy*um)
    ], width = p.coursegatewidth*um)
    ...
    gate_trace_13 = db.Path([((+0.5*p.wHallbar)*um, 0.16*p.lHallbar*um),
                ((+0.5*p.wHallbar+p.w_connector)*um, 0.22*p.lHallbar*um),
                ((+0.5*p.wHallbar+p.w_connector)*um, 0.22*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+p.dohmicsx)*um,  0.22*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+p.dohmicsx)*um, 0.22*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+2*p.dohmicsx)*um, 0.22*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+2*p.dohmicsx)*um, 0.22*p.lHallbar*um),
                ((+0.5*p.wHallbar+2*p.w_connector+2.5*p.dohmicsx)*um, 0.22*p.lHallbar*um)
    ...
    gate_trace_14 = db.Path([((+0.5*p.wHallbar)*um, 0.32*p.lHallbar*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector)*um, 0.32*p.lHallbar*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector)*um, (0.20*p.lHallbar+0.20*p.dohmicsy)*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector+p.dohmicsx)*um, (0.20*p.lHallbar+0.20*p.dohmicsy)*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector+p.dohmicsx)*um, (0.45*p.lHallbar+0.20*p.dohmicsy)*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector+2*p.dohmicsx)*um, (0.45*p.lHallbar+0.20*p.dohmicsy)*um),
                ((+0.5*p.wHallbar+1.25*p.w_connector+2*p.dohmicsx)*um, (0.55*p.lHallbar+0.20*p.dohmicsy)*um),
                ((+0.5*p.wHallbar+1*p.w_connector+2.5*p.dohmicsx)*um, (0.55*p.lHallbar+0.20*p.dohmicsy)*um)
    ], width = p.coursegatewidth*um)
    ...
    fanout8 = db.Path([db.Point(list(gate_trace_8.each_point())[0].x+1*um, list(gate_trace_8.each_point())[0].y), 
                       db.Point(2.0*p.gate_separation*um, -0.14*p.lHallbar*um),
                       db.Point(1.25*p.gate_separation*um, -1.25*p.gate_separation*um),
                       ], width = 0.5*p.coursegatewidth*um)
    fanout9 = db.Path([db.Point(list(gate_trace_9.each_point())[0].x+1*um, list(gate_trace_9.each_point())[0].y), 
                       db.Point(2.5*p.gate_separation*um, -0.11*p.lHallbar*um),
                       db.Point(1.5*p.gate_separation*um, -1.0*p.gate_separation*um),
                       ], width = 0.5*p.coursegatewidth*um)
    fanout10 = db.Path([db.Point(list(gate_trace_10.each_point())[0].x+1*um, list(gate_trace_10.each_point())[0].y), db.Point(1*p.gate_separation*um, 0*p.gate_separation*um)], width = 0.5*p.coursegatewidth*um)

    fanout11 = db.Path([db.Point(list(gate_trace_11.each_point())[0].x+1*um, list(gate_trace_11.each_point())[0].y), 
                       db.Point(2.5*p.gate_separation*um, 0.11*p.lHallbar*um),
                       db.Point(1.5*p.gate_separation*um, 1.0*p.gate_separation*um),
                       ], width = 0.5*p.coursegatewidth*um)
    
    fanout12 = db.Path([db.Point(list(gate_trace_12.each_point())[0].x+1*um, list(gate_trace_12.each_point())[0].y), 
                       db.Point(2.0*p.gate_separation*um, 0.14*p.lHallbar*um),
                       db.Point(1.25*p.gate_separation*um, 1.25*p.gate_separation*um),
                       ], width = 0.5*p.coursegatewidth*um)

    fanout13 = db.Path([db.Point(list(gate_trace_13.each_point())[0].x+1*um, list(gate_trace_13.each_point())[0].y), db.Point(1.5*p.gate_separation*um, list(gate_trace_13.each_point())[0].y), db.Point(1.0*p.gate_separation*um, 1.5*p.gate_separation*um)], width = 0.5*p.coursegatewidth*um)
    fanout14 = db.Path([db.Point(list(gate_trace_14.each_point())[0].x+1*um, list(gate_trace_14.each_point())[0].y),db.Point(0.5*p.gate_separation*um, list(gate_trace_14.each_point())[0].y), db.Point(0.5*p.gate_separation*um, 4*p.gate_separation*um)], width = 0.5*p.coursegatewidth*um)
    ...
    gatepads_rgn.insert(fanout11)
    gatepads_rgn.insert(fanout12)
    gatepads_rgn.insert(fanout13)
    gatepads_rgn.insert(fanout14)
    ...
    gatepads_rgn.insert(patch.transformed(db.ICplxTrans(1, 30, False, list(fanout8.each_point())[-1].x, list(fanout8.each_point())[-1].y)))
    gatepads_rgn.insert(patch.transformed(db.ICplxTrans(1, 35, False, list(fanout9.each_point())[-1].x, list(fanout9.each_point())[-1].y)))

    gatepads_rgn.insert(patch.transformed(db.ICplxTrans(1, 90, False, list(fanout10.each_point())[-1].x, list(fanout10.each_point())[-1].y)))
    gatepads_rgn.insert(patch.transformed(db.ICplxTrans(1, -50, False, list(fanout11.each_point())[-1].x, list(fanout11.each_point())[-1].y)))
    gatepads_rgn.insert(patch.transformed(db.ICplxTrans(1, -40, False, list(fanout12.each_point())[-1].x, list(fanout12.each_point())[-1].y)))

    gatepads_rgn.insert(patch.transformed(db.ICplxTrans(1, -30, False, list(fanout13.each_point())[-1].x, list(fanout13.each_point())[-1].y)))
    gatepads_rgn.insert(patch.transformed(db.ICplxTrans(1, -5, False, list(fanout14.each_point())[-1].x, list(fanout14.each_point())[-1].y)))
    ...
    gatepads_rgn.insert(gate_trace_11)
    gatepads_rgn.insert(gate_trace_12)
    gatepads_rgn.insert(gate_trace_13)
    gatepads_rgn.insert(gate_trace_14)
