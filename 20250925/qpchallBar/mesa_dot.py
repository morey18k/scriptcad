...
def make_mesaf_1(um, p):
    ...
    trenchaddon = db.Box((0.5*(p.wHallbar-p.wWire)+overlap)*um, p.lWire*um)
    region.insert(trenchaddon.transformed(db.ICplxTrans.new(1, 0, False, 0.5*p.wWire*um+0.25*(p.wHallbar-p.wWire+2*overlap)*um, 0)))
    region.insert(trenchaddon.transformed(db.ICplxTrans.new(1, 0, False,  -0.5*p.wWire*um-0.25*(p.wHallbar-p.wWire+2*overlap)*um, 0)))
