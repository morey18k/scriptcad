...
def make_mesa_ohmics(um, p):
    ...
    for k, center in enumerate(pad_centers_y):
        if k == 0 or k == len(pad_centers_y)-1:
            ...
            mesa_region.insert(region.transformed(db.ICplxTrans.new(1, 0, False, 1.*p.dohmicsx*um, center*um)))
            mesa_region.insert(region.transformed(db.ICplxTrans.new(1, 0, False, 2*p.dohmicsx*um, center*um)))
    ...
    probe_points = [((-0.5*p.wHallbar-0.5*p.w_connector)*um, -0.4*p.lHallbar*um)
                    , ((0)*um, (-0.5*p.lHallbar-0.5*p.h_connector)*um)
    ...
    ohmic_trace_1 = db.Path([((-0.5*p.wHallbar-0.5*p.w_connector)*um, -0.4*p.lHallbar*um), 
                             ((-0.5*p.wHallbar-1.5*p.w_connector)*um, -0.4*p.lHallbar*um),
                             ((-0.5*p.wHallbar-1.5*p.w_connector)*um, -0.75*p.lHallbar*um),
    ...
    ohmic_trace_4 = db.Path([((0.5*p.wHallbar+0.5*p.w_connector)*um, -0.24*p.lHallbar*um), 
                             ...
                             ((0.5*p.wHallbar+p.w_connector)*um, -0.17*p.lHallbar*um),
                             ((1.8*p.dohmicsx)*um, -0.17*p.lHallbar*um),
                             ((1.8*p.dohmicsx)*um, -0.25*p.lHallbar*um)
                             ], width = 15*um)
    ...
    ohmic_trace_6 = db.Path([((0.5*p.wHallbar+0.5*p.w_connector)*um, 0.08*p.lHallbar*um), 
                             ...
                             ((0.5*p.wHallbar+p.w_connector)*um, 0.135*p.lHallbar*um),
                             ((1.8*p.dohmicsx)*um, 0.135*p.lHallbar*um),
                             ((1.8*p.dohmicsx)*um, 0*p.lHallbar*um)
                             ], width = 15*um)
    
    ohmic_trace_7 = db.Path([((0.5*p.wHallbar+0.5*p.w_connector)*um, 0.24*p.lHallbar*um), 
                             ((0.5*p.wHallbar+p.w_connector)*um, 0.24*p.lHallbar*um),
                             ((0.5*p.wHallbar+p.w_connector)*um, 0.18*p.lHallbar*um),
                             ((1.8*p.dohmicsx)*um, 0.18*p.lHallbar*um),
                             ((1.8*p.dohmicsx)*um, 0.25*p.lHallbar*um)
                             ], width = 15*um)
    ...
    ohmic_trace_10 = db.Path([((-0.5*p.wHallbar-0.5*p.w_connector)*um, 0.4*p.lHallbar*um), 
                             ((-0.5*p.wHallbar-1.5*p.w_connector)*um, 0.4*p.lHallbar*um),
                             ((-0.5*p.wHallbar-1.5*p.w_connector)*um, 0.75*p.lHallbar*um),
                             (1.8*p.dohmicsx*um, 0.75*p.lHallbar*um),
                             ((1.8*p.dohmicsx)*um, 0.6*p.lHallbar*um)
...
def make_mesa_gatepads(um, p):
    ...
    for center in pad_centers_y:
        ...
        mesa_region.insert(gate_pad.transformed(db.ICplxTrans.new(1, 0, False, -1.1*p.dohmicsx*um, center*um)))
