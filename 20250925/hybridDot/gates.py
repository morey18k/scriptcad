...
def create_gatepads(um, p):
    ...
    c = gf.Component()    
    lgth = 1
    w1 = gf.c.straight(length = lgth, cross_section = "metal_routing", width = 0.5*p.coursegatewidth)
    rtop = c << w1
    rmiddle = c << w1
    rbottom = c << w1
    ...
    c.remove_layers(layers=[LAYER.PORT])

    fanout = db.Region(c.get_polygons()[24])
